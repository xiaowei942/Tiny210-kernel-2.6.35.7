...
{
	//此硬件控制只是根据传递进来的ctrl标志（软件标志）进行相应的操作，操作
	//过程实际就是使能芯片后将要发送的内容送进相应寄存器，即可由Nand Flash
	//控制器自动完成时序的创建，实现相应功能。
	unsigned int cur;
	void __iomem *regs = s3c_nand.regs;
...
static int s3c_nand_read_page_1bit(struct mtd_info *mtd, struct nand_chip *chip,
		uint8_t *buf)////////////////////  COMMENT BY WEI ///////////////////, int page)
{
...
static int s3c_nand_probe(struct platform_device *pdev, enum s3c_cpu_type cpu_type)
{
	...
	//如果定义MTD分区，则将分区表置空，并将分区数设为0
	struct mtd_partition *partitions = NULL;
	int num_partitions = 0;
#endif

#if defined(CONFIG_MTD_NAND_S3C_HWECC)
	//如果定义nand的ECC校验方式为硬件ECC，则添加nand_flash_dev结构
	...
	//通过platform_device的device成员，获取nand的时钟源，并激活
	s3c_nand.clk = clk_get(&pdev->dev, "nand");
	//IS_ERR最终实现为unlikely()，表示if判断结果为假的可能性更大点，并将可能性大的代码优化为就近链接
	...
	//将platform_device的resource传递给res这个指针变量
	res  = pdev->resource;
	//获取到resource的空间大小
	size = res->end - res->start + 1;

	//通过上面获取的大小以及platform设备名称，申请相应内存到s3c_nand_info中的area成员（resource类型）
	...
	//将传递进来的cpu_type保存到s3c_nand_info的cpu_type成员
	s3c_nand.cpu_type   = cpu_type;
	//将传递进来的platform_device的dev成员保存到s3c_nand_info的device成员
	s3c_nand.device     = &pdev->dev;

	//将res内存段(实际上是寄存器的物理内存地址空间)映射到虚拟地址空间以便操作
	...
	//为mtd_info和nand_chip同时分配内存（两结构的内存前后连续）
	...
	//跳过s3c_mtd后就是nand_chip的内存起始位置，此位置可用s3c_mtd[1]访问
	...
	//将mtd_info的私有数据指针指向nand
	s3c_mtd->priv = nand;
	
	//将自定义的s3c_nand_mtd_info的chip_nr变量值（芯片数）个芯片
	for (i = 0; i < plat_info->chip_nr; i++) {
		...
		nand->options		|= NAND_NO_SUBPAGE_WRITE;   //////////// WEI ///////////= 0;
		nand->badblockbits	= 8;
		...
		tmp = readb(nand->IO_ADDR_R); /* Maf. ID ---------应该为：0xECh */
		printk(KERN_INFO "NandFlash Maker Code: 0x%x\n",tmp);
		
		tmp = readb(nand->IO_ADDR_R); /* Device ID -------应该为：0xDAh */
		printk(KERN_INFO "NandFlash Device Code: 0x%x\n",tmp);
		
		//搜索Nand Flash的id列表
		//列表定义在drivers/mtd/nand/nand_ids.c中
		//根据刚刚读出的Device ID匹配设备
		...
		nand->cellinfo = readb(nand->IO_ADDR_R);	/* the 3rd byte 见K9F2G08U0A Flash手册32页”3rd ID Data“表*/
		printk(KERN_INFO "NandFlash 3rd ID Data: 0x%x\n",nand->cellinfo);
		
		tmp = readb(nand->IO_ADDR_R);			/* the 4rd byte 见K9F2G08U0A Flash手册32页”4rd ID Data“表*/
		printk(KERN_INFO "NandFlash 4th ID Data: 0x%x\n",tmp);

		if (!type->pagesize) {
			if (((nand->cellinfo >> 2) & 0x3) == 0) {// 2 Level Cell is SLC
				printk("NandFlash type: SLC\n");
				nand_type = S3C_NAND_TYPE_SLC;
				nand->ecc.size = 512;
				nand->ecc.bytes	= 4;

				/*  For 4KB Page 8_bit ECC */
				if ((1024 << (tmp & 3)) == 4096) { //即：4rd byte 的最低2位为0b10，也就是pagesize＝4k
					...
					if ((1024 << (tmp & 0x3)) > 512) {
						...
						printk("Ecc layout 16\n");
						nand->ecc.layout = &s3c_nand_oob_16;
					}
				}
			} else {
				printk("NandFlash type: MLC\n");
				nand_type = S3C_NAND_TYPE_MLC;
				nand->options |= NAND_NO_SUBPAGE_WRITE;	/* NOP = 1 if MLC */

#ifdef CONFIG_MACH_MINI210
				////// WEI /////nand->badblockbits = 4;
				////// WEI /////s5p_nand_mlc_probe(nand, s3c_nand.regs);
#else
				if ((1024 << (tmp & 3)) == 4096) {
					/* Page size is 4Kbytes */
					nand->ecc.read_page = s3c_nand_read_page_8bit;
